...
const toBuffer = hexString => {
  ...
  return Uint8Array.from(hexString.match(/.{1,2}/g).map(b => parseInt(b, 16)));
};

const toHex = buffer => "0x" + Array.from(buffer).map(b => b.toString(16).padStart(2, "0")).join("");
...
const computeEthereumBlockHash = async block => {
  ...
  const rlpEncoded = rlp.encode(header);
...
const loadRealBlocksWithFullHeader = async () => {
  ...
  console.log("Loaded blocks (full data):", blocks);
...
const renderBlockchain = (blocks) => {
  ...
  blocks.forEach((block, idx) => {
    ...
    blockDiv.style.transition = "border-color 0.3s";
    ...
    if (block.transactions.length > 0) {
      ...
      try {
        ...
        functionName = "Unknown (possibly constructor)";
      }
    }

    blockDiv.innerHTML = `
      <div><label>Block #:</label><input class="form-control form-control-sm mb-2" value="${blockNumber}"></div>
      <div><label>Timestamp:</label><input class="form-control form-control-sm mb-2" value="${timestamp}"></div>
      <div><label>Miner:</label><input class="form-control form-control-sm mb-2" value="${block.miner}"></div>
      <div><label>Parent Hash:</label><input class="form-control form-control-sm mb-2" value="${block.parentHash}"></div>
      <div><label>Transactions Count:</label><input class="form-control form-control-sm mb-2" value="${block.transactions.length}"></div>
      <div><label>Blockchain Hash:</label><input class="form-control form-control-sm mb-2" value="${block.hash || "N/A"}"></div>
  ...
  // Monitor changes and mark subsequent blocks as "out of sync"
  const blocksDivs = container.querySelectorAll(".block");
  blocksDivs.forEach((blockDiv, idx) => {
    const inputs = blockDiv.querySelectorAll("input");
    inputs.forEach(input => {
      input.addEventListener("input", () => {
        for (let j = idx + 1; j < blocksDivs.length; j++) {
          blocksDivs[j].style.borderColor = "red";
        }
      });
      input.addEventListener("change", () => {
        // If you want to validate and reset red styling when value is "back to normal", do it here.
      });
    });
  });
};
...
const updateTotalSupply = async () => {
  try {
    ...
    document.getElementById("totalSupply").innerText = parseFloat(ethers.utils.formatUnits(rawSupply, 18)).toFixed(4);
  } catch (err) {
...
const updatePauseState = async () => {
  try {
    ...
    document.getElementById("pauseState").innerText = paused ? "Paused" : "Active";
    document.getElementById("togglePause").innerText = paused ? "Unpause Contract" : "Pause Contract";
  } catch (err) {
...
document.getElementById("send").onclick = async () => {
  ...
  await handleTransaction(() => token.transfer(recipient, ethers.utils.parseUnits(amount, 18)),
...
document.getElementById("mint").onclick = async () => {
  ...
  if (!ethers.utils.isAddress(to)) return showToast("Invalid mint address!", "error");
  if (!amount || isNaN(amount) || parseFloat(amount) <= 0) return showToast("Invalid amount!", "error");

  await handleTransaction(() => token.mint(to, ethers.utils.parseUnits(amount, 18)),
...
document.getElementById("burn").onclick = async () => {
  ...
  if (!ethers.utils.isAddress(from)) return showToast("Invalid burn address!", "error");
  if (!amount || isNaN(amount) || parseFloat(amount) <= 0) return showToast("Invalid amount!", "error");

  await handleTransaction(() => token.burn(from, ethers.utils.parseUnits(amount, 18)),
...
document.getElementById("blacklist").onclick = async () => {
  ...
  if (!ethers.utils.isAddress(address)) return showToast("Invalid address!", "error");

  await handleTransaction(() => token.blacklist(address, true),
...
document.getElementById("unblacklist").onclick = async () => {
  ...
  await handleTransaction(() => token.blacklist(address, false),
